...
毎週金曜 午後5時（17時・JST）に Claude Code routine として自動実行される、作り置き献立の生成・公開システム。
...
| `menu.json` | 今週の生成レシピデータ（写真URL含む） | 毎週金曜に上書き |
...
- スケジュール実行タイミング：**毎週金曜 午後5時（17時・JST）**
...
- `recipes[]` で `super_count > 0`：**殿堂入り（超いいね）**。家族の特別なお気に入り。食材が合う週は候補へ入れて再登場させてよいが、**バイアス防止の上限**を必ず守る：
  - **クールダウン**：そのレシピの `last_adopted_at`（前回採用日）から**14日以上**あいていない場合は採用しない
  - **枠上限**：殿堂入り由来の採用は**1週あたり最大2品**まで（残りの枠は常に新規・通常候補に使う）
  - 前週との意味的重複除外は通常どおり適用。優先度は priority > 殿堂入り > 通常
...
**収集の時点から人気度を勘案する（重要）。** 人気メニューは候補プールに入っていて初めて選べる。Step4の保存数ランクは「集めた候補の中」でしか効かないため、集める段階で人気に寄せておく：
- **各軸のクエリに必ず `人気` を入れる**（下記の具体例で `人気` が無い行も、実行時は `人気` を付けて検索する）
- 下記 **軸K（人気ランキング特化）** を必ず実施する
- **検索上位のヒットを優先的に候補プールへ入れる**（検索順位は人気の粗い代理指標）
...
**K. 人気ランキング特化（必須）**
- 目的：人気メニューそのものを候補プールに確実に入れる
- パターン：`{今週の主要食材} 人気 ランキング 作り置き site:kurashiru.com` ／ `{今週の主要食材} 殿堂 作り置き site:kurashiru.com`
- 具体例：`鶏もも肉 人気 ランキング 作り置き site:kurashiru.com`
- 具体例：`豚こま切れ 人気 ランキング 作り置き site:kurashiru.com`
- 具体例：`鶏むね肉 殿堂 作り置き site:kurashiru.com`
- ヒットした「人気レシピまとめ／ランキング」ページに載る個別レシピURLも候補として控える

---

### 検索の最低条件
- A〜D・H〜K を合わせて**合計60件以上**の候補名を収集してから次に進む（各ジャンル主菜4品ずつ選ぶため多めに集める）
- 60件未満の場合はキーワードを変えて追加検索する
- 和・洋・中・エスニックそれぞれ**最低7件以上**確保できていること（4品選定＋フィルタ脱落分の余裕）。不足するジャンルは H〜J の該当軸で追加検索する
- **各枠、少なくとも数件は軸K（人気ランキング）由来の候補を含める**。Step4で保存数上位から選べるよう、有力な人気候補をプールに確保しておく

### 検索の使い方
- 得られたレシピ名・概要を**参考候補**として参照する
- 収集段階では手順・分量の詳細は不要（レシピ名・URL・概要だけ控える）。**クラシル候補の詳細は選定後に Step 5 の `kurashiru_recipe.py` で実レシピから取得する**。白ごはん.com はページ取得不可（許可外）のため、選ばれた場合のみ Claude が家庭料理の知識で補完する
- 両サイトの候補から選びつつ、以下のルールに従って最終的な献立を決定する
- **クラシル候補は検索結果の URL（`https://www.kurashiru.com/recipes/...`）も控えておく**。後段の人気度スコア（保存数）取得に使う
- **人気度は2段階で効かせる**：①収集時＝各クエリの `人気`・軸K・検索上位でプールを人気寄りにする（粗い）／②選定時＝`kurashiru_rank.py` の保存数で定量ランク（精密）。①でプールに人気候補が入っていないと②が活きないので、収集を疎かにしない
...
| 枠 | 数 | ジャンル | 候補の条件 |
|----|---|---------|----------|
| 主菜（和） | 4品 | 和 | 醤油・みりん・味噌・だし系 |
| 主菜（洋） | 4品 | 洋 | トマト・クリーム・バター・オリーブオイル系 |
| 主菜（中） | 4品 | 中 | 中華調味料（豆板醤・オイスター・ごま油・甜面醤）系 |
| 主菜（エスニック） | 4品 | エスニック | ナンプラー・ガパオ・カレー粉・コチュジャン等 |
| 副菜 | 4品 | 問わず | 各2〜3日分 |

- 主菜の各ジャンル（和・洋・中・エスニック）でビストロタグの候補を**必ず1品以上**含める（計4品以上）
- **主菜の各ジャンル（和・洋・中・エスニック）で、ミールキット型（作り置き適性B＝下味冷凍、またはC＝成形済み冷凍）を必ず1品以上含める（計4品以上・`freeze: true`）。** 平日を「解凍して焼く/炒めるだけ」で完成させるため。ジャンルごとに、そのジャンルで自然な下味冷凍/成形冷凍を選ぶ（例：和＝下味冷凍生姜焼き、洋＝成形冷凍ハンバーグ、中＝下味冷凍油淋鶏、エスニック＝下味冷凍タンドリー/ヤンニョム 等）
- 1品が「ビストロ枠」と「ミールキット枠」を兼ねることはできない（ビストロ煮込みは冷凍前提の下味冷凍ではないため）。各ジャンルは最低でもビストロ1品＋ミールキット1品を別々に確保する
- `priority === true` のレシピを主菜か副菜の適切な方に必ず1品含める
- **献立は「配達食材から作れるレシピのカタログ」である（重要）。** ユーザーは提示された中から作るものを選ぶため、**品数は必ず規定数（主菜16・副菜4）を揃える**
  - **メインの食材（肉・魚・野菜・豆腐等）は、配達食材・残り食材・常備ハーブにあるものだけを使う**。配達に無い食材の買い足し前提レシピは作らない
  - 調味料・油・片栗粉・小麦粉・だし・ごま等の基礎材料は常備前提で自由に使ってよい
  - **同じ配達食材を複数のレシピで重複して使ってよい**（全部作るわけではないので、量の奪い合い・使い切りは考えない）。例：豚こま300gの週に「豚こまレシピ」を5品載せてよい
  - **「食材が足りないから品数を減らす」判断は禁止**
...
1. 各枠でフィルタ（作り置き適性・重複除外）を通過したクラシル候補のURLを集め、次を実行：
   ...
   **A. 採用する料理は「その料理の最も人気なバージョン」の保存数を記録する（必須）。** クラシルには**同じ料理が多数の別URL**で存在し、保存数が10万のものも数百のものもある。たまたま拾ったURLの数字を記録すると、料理間で「10万と200」のような極端なブレが出る。これを防ぐため、**ある料理を採用すると決めたら、その料理名で `{料理名} 人気 site:kurashiru.com` を検索して同一料理のクラシルURLを2〜4件集め、`kurashiru_rank.py` にまとめて渡し、`bookmark_count` が最大のバージョンを採用URL・採用数値とする**（＝各料理は常に「最人気版」の保存数で並ぶ）。`source_url` もその最人気版のURLにする。
2. 各枠は原則、**保存数の多い候補から選ぶ**（人気メニューを定量的に優先）
3. **品質ゲート**：`rating_value < 4.2` は人気でも避ける。`rating_count` が極端に少ない（例：30未満）ものは数値の信頼性が低いので過信しない

   **B. 下限ゲート（保存数の底上げ・必須）：** 上記Aで最人気版を取っても `bookmark_count` が **5,000未満** の場合、その数字は「人気の裏付け」として弱いため **`popularity` フィールドを付けない**（＝アプリに人気度チップを出さない）。料理自体は採用してよい（配達食材で作れる定番なら献立に載せる価値はある）が、**弱い数字を人気バッジとして表示しない**ことで、カード間の「10万と200」のようなブレをなくす。＝人気度チップが出る料理はすべて5,000保存以上に揃う。
...
選定した20品（主菜16・副菜4）それぞれについて詳細を作成する：
- 手順（3〜5ステップ）。調理器具の具体的な設定値を含めること。**元レシピの分量・技法に忠実**にしつつ、3人前に換算する
...
- 分量はすべて**3人前**（大人2人＋子供1人の夜ごはん1食分相当）で計算すること
...
## お願い
上記の配達食材から作れるレシピのカタログを、以下の形式でJSONのみ返してください。
**この献立は「選ぶためのカタログ」です。メインの食材は配達食材（＋残り食材・常備ハーブ）に限定し、調味料・油・粉類は常備前提で自由。同じ食材を複数レシピで使い回してよく（全品を作る前提ではないため量の整合は考慮不要）、必ず規定の品数を揃えてください。**

- 主菜16品・副菜4品
- **主菜は和4品・洋4品・中4品・エスニック4品の構成にすること**
- 各ジャンルでビストロタグのレシピを必ず1品以上含めること（エスニックは1品でよい）
- **各ジャンル（和・洋・中・エスニック）で、ミールキット型（B下味冷凍 または C成形冷凍・`freeze: true`）を必ず1品ずつ含めること（計4品）。平日に解凍して焼く/炒めるだけで完成する状態にする。ビストロ枠とは別の品にすること**
...
- 分量はすべて**3人前**（大人2人＋子供1人の夜ごはん1食分相当）で計算する
...
- 3〜5単語に収める
- **全品（主菜16品・副菜4品）すべてで異なるキーワードになるよう意識する**。複数の副菜で `salad` `dressed` `sesame` `marinated` などの汎用語が重複すると同じ画像になりやすいため避ける
...
- **採用メタデータの記録**：新しい `menu.json` に登場したリストレシピの `last_adopted_at`（＝delivery_date）と `done_count` を更新する（殿堂入りの14日クールダウン判定の根拠。同じ配達日への再実行では二重カウントしない）

**注意：上記の採用メタデータ以外、`recipes[]` には触れない。** レシピリストはユーザーが👍を押したか手動登録したものだけで構成される。ルーティンが自動追加・削除しない。
